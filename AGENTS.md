...
./deploy/deploy-production.sh       # restart both services + health verification
```

- `deploy/deploy-production.sh` is the canonical production restart path: it
  restarts both services and verifies web HTTP 200 plus `[build-worker] ready`.
  The live design canary is NOT part of it — a deploy must not be blocked by a
  provider/budget failure while the pipeline is being stabilised. Run the
  real-provider gate manually when you want it: `./deploy/verify-design-canary.sh`
  drives all 34 curated briefs through the live LLM pipeline under the production
  `.env` profile and per-run budget, and exits nonzero unless every brief commits
  all five stages. Unit tests do not exercise the provider/budget path.
