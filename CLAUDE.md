...
| `kicraft/autoplacer/` | Placement + routing engine (the geometry/DRC core). `brain/placement_solver.py`, `brain/subcircuit_composer.py`, `routing_board.py`, `brain/leaf_routing.py`, `brain/gnd_pour.py`, `brain/breakout_stubs.py`. **Treat as load-bearing; surgical fixes only.** |
