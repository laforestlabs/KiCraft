# KiCraft Wiring — Implementation Spec (v1)

A spec for making KiCraft synthesis produce **fully wired, human-readable** KiCad 9 schematics from a `ConversationState`. The pipeline today places components and emits hierarchical labels but draws no wires, no junctions, and no power symbols; the resulting schematics are unroutable and unreadable. This spec defines the complete fix end to end — no shortcuts, no stub-with-label hacks.
...
`kicraft synthesize` emits a KiCad 9 hierarchical schematic with the right sheet structure: root sheet + N leaf sheets, correct sheet pins, correct refdes uniqueness, all symbols/footprints resolvable in stock KiCad 9 libraries. Validators §9.1–§9.8 all pass.

### What's broken (the one real blocker)

Every leaf sheet from the from-scratch path (`_emit_leaf` in `kicraft/design/synthesis/emitter.py:255-310`) contains:
...
│ cli_app.py: + `kicraft lookup-symbol <Lib:Name>`            │
└─────────────────────────────────────────────────────────────────────────┘
              │
              ▼
┌─────────────────────────────────────────────────────────────────────────┐
...
## 5. Schema additions (`kicraft/design/models.py`)
...
Read the current `.kicraft/state.json`. For every part, run:
    ...
    kicraft lookup-symbol <Library:Symbol>
...
Add to `kicraft/design/cli_app.py`:
...
New module: `kicraft/design/synthesis/symbol_pinout.py`.
...
New module: `kicraft/design/synthesis/placement.py`.
...
New module: `kicraft/design/synthesis/router.py`.
...
## 10. Emitter changes (`kicraft/design/synthesis/emitter.py`)
...
## 11. PCB population (`kicraft/design/synthesis/kicad_pcb_stub.py`)
...
## 12. New validators (`kicraft/design/synthesis/validation.py`)
...
- Add `kicraft lookup-symbol` subcommand.
- Tests for `lookup_pins` covering R, C, ESP32 module, extends-resolved symbols, missing symbol.

Ship criterion: `kicraft lookup-symbol Device:R` returns the expected JSON.
...
Ship criterion: re-running `kicraft synthesize` on `ESP32_LED_ACCEL` produces 9 leaf sheets with non-zero wire counts and KiCad opens them without errors.
