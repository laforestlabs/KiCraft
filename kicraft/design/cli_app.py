...
def _cmd_validate_part(args: argparse.Namespace) -> int:
    ...
    Checks, in order: (1) the manifest parses and the directory
...
def _run_layout(
    ...
    """
    # Bring the project's DRC floors down to the fab capability BEFORE any
    # engine loads a board. Every leaf/parent board is written by copying the
    # project's .kicad_pro forward, so this one call is what makes the
    # deterministic escape copper (0.35/0.2 fanout vias, capability-width escape
    # tracks) legal to KiCad DRC. It matters most on `replay`, whose seed
    # project was synthesised under the old floors and would otherwise silently
    # test the rules the change was meant to move. Only ever lowers.
    try:
        ...
        _proj = pcb.resolve().parent
        _found = discover_project_config(_proj)
        stamp_fab_floors_for_project(
            _proj,
            {**DEFAULT_CONFIG, **(load_project_config(str(_found)) if _found else {})},
        )
    except Exception as exc:  # noqa: BLE001 -- never block a build on this
        print(f"[build]   note: fab-floor stamp skipped: {exc}")
...
def _shipped_advisory_codes(state) -> list[str]:
    ...
    if state is None:
        return []
    return recorded_advisory_codes(
        getattr(state, "architecture", None), getattr(state, "bom", None)
    )
