...
# The architecture stage answers ONE large document for ~thirty rules, and its refusals are
# interactions between sections rather than one missing rule: a repair that fixes the contact list
# breaks the net binding, the next attempt fixes that and breaks something else. The drive therefore
# starts with the whole-slot contract (`kicraft_architecture_response_v2`) and, when that answer is
# refused or incomplete, asks for ONE section at a time with the small contract below, then
# re-validates the assembled document. The order is a dependency order: parts reference sheets,
# signals reference parts, obligation ownership references requirements.
ARCHITECTURE_SUB_STEPS: tuple[str, ...] = ("sheets", "parts", "signals", "obligations")

# The intent fields each section owns. A section answer that carries more than its own fields is
# absorbed, never punished: the assembly re-validates the whole document after every merge, so a
# provider that answers everything at once simply finishes the assembly in one call (the reference
# corpus' recorded whole payload does exactly that).
_ARCHITECTURE_SECTION_FIELDS: dict[str, tuple[str, ...]] = {
    "sheets": (
        "topologies",
        "comms_protocols",
        "mcu_present",
...
_ARCHITECTURE_INTENT_FIELDS = frozenset(
    field for fields in _ARCHITECTURE_SECTION_FIELDS.values() for field in fields
) - {"owners"}

# The refusal names the section that owns its fix, by its own deterministic codes (never by prose
# keywords: the aggregate `multiple_intent_contracts` message contains every word, so a keyword scan
# asks the wrong section and burns the rung's budget — measured on the 2026-09-17 bounded campaign,
# where `declared_signal_port_tied` about an mcp23017 `declared_ports` entry asked the signals
# section three times). Every code is one the architecture stage actually raises
# (`kicraft.design.architecture_intent`, the recipe resolver, and this module's own validators); a
# code the table does not name is not guessed at — only the sections the answer still owes are asked.
_ARCHITECTURE_CODE_SECTIONS: dict[str, str] = {
    # The signals section names the nets and their endpoints; the compiler binds the ports.
    "conflicting_port_binding": "signals",
    "unbound_required_port": "signals",
    "malformed_signal_ref": "signals",
    "incomplete_usb_edge": "signals",
    "signal_names_rail": "signals",
    "signal_conflicts_with_rail": "signals",
    "unknown_edge_rail": "signals",
    "unknown_signal_requirement": "signals",
    "duplicate_edge_connector": "signals",
    "empty_edge_connector": "signals",
    "derived_output_name_collision": "signals",
    "recipe_signal_in_power_nets": "signals",
    # The parts section states each requirement: family, parameters, supply and its claimed pins.
    "declared_signal_port_tied": "parts",
    "declared_port_double_bound": "parts",
    "duplicate_requirement_id": "parts",
    "unknown_interface_port": "parts",
    "unknown_tie_net": "parts",
    "unknown_supply_rail": "parts",
    "conflicting_supply_binding": "parts",
    "conflicting_reference_binding": "parts",
    "unknown_reference_domain": "parts",
    "reference_domain_not_zero_volt": "parts",
    "unsupported_lowerer_contract": "parts",
    "invalid_lowerer_parameters": "parts",
    "unrealizable_power_requirement": "parts",
    "unknown_part_refused": "parts",
    "unsupported_protected_variant": "parts",
    "missing_recipe_requirement": "parts",
    "protected_recipe_unavailable": "parts",
    "recipe_output_port_collision": "parts",
    "conflicting_recipe_parameter": "parts",
    "conflicting_recipe_supply": "parts",
    "conflicting_exact_variant": "parts",
    "conflicting_programming_contract": "parts",
    "incompatible_programming_power_domain": "parts",
    "usb_connector_supply_unknown": "parts",
    # The sheets section owns the sheet list, the rails and the chosen standard form factor.
    "unknown_requirement_sheet": "sheets",
    "contradictory_standard_form_factor": "sheets",
    "unsupported_standard_stacking_interface": "sheets",
    "incomplete_standard_stacking_interface": "parts",
    "invalid_standard_stacking_owner": "parts",
    "invalid_standard_stacking_pinmap": "parts",
    # The obligations section owns where each committed row is implemented.
    "source_obligation_not_retained": "obligations",
}


def architecture_rejection_codes(diagnostic: dict | None) -> tuple[str, ...]:
    """Every code a refusal names, its own and the ones in its evidence rows."""
    if not isinstance(diagnostic, dict):
        return ()
    codes = [str(diagnostic["code"])] if diagnostic.get("code") else []
    codes.extend(
        str(row["code"])
        for row in diagnostic.get("evidence") or []
        if isinstance(row, dict) and row.get("code")
    )
    return tuple(dict.fromkeys(codes))


class ArchitectureObligationOwner(BaseModel):
    """One committed obligation and the requirement(s) that implement it."""

    model_config = ConfigDict(extra="forbid")

    kind: Literal["physical", "quantity", "adjustability", "conversion", "quantitative"]
    original_obligation_id: str = Field(pattern=r"^[a-z][a-z0-9_-]{0,63}$")
    requirement_ids: list[str] = Field(min_length=1)


class ArchitectureObligationsResponse(BaseModel):
    """The architecture answer to "who implements each committed obligation"."""

    model_config = ConfigDict(extra="forbid")

    owners: list[ArchitectureObligationOwner] = Field(min_length=1)
...
def _architecture_section_schema(sub_step: str) -> dict:
    """One section of the architecture intent, as its own provider schema.

    The section schema is a subset of `ArchitectureIntent`'s own JSON schema, so the enums, patterns
    and descriptions stay defined in exactly one place; `requirements` keeps the whole
    `IntentRequirement` shape, because a part's family, parameters, claimed pins and ties are the
    facts only that section can state.
    """
    if sub_step == "obligations":
        return ArchitectureObligationsResponse.model_json_schema()
    from kicraft.design.architecture_intent import ArchitectureIntent

    schema = ArchitectureIntent.model_json_schema()
    fields = _ARCHITECTURE_SECTION_FIELDS[sub_step]
    definitions = schema.get("$defs") or {}
    properties = {name: schema["properties"][name] for name in fields}
    section = {
        "type": "object",
        "properties": properties,
        "required": sorted(name for name in fields if name in (schema.get("required") or ())),
    ...
    if "requirements" in properties:
        properties["requirements"]["minItems"] = 1
        section["required"] = sorted({*section["required"], "requirements"})
        requirement = definitions.get("IntentRequirement") or {}
        requirement_properties = requirement.get("properties") or {}
        functional_blocks = requirement_properties.get("functional_blocks")
        if isinstance(functional_blocks, dict):
            functional_blocks["minItems"] = 1
...
def architecture_section_contract(
    sub_step: str,
    prompt_state: dict,
    *,
    allow_questions: bool = False,
) -> StageResponseContract:
    """The small provider contract for one architecture section call."""
    if sub_step not in ARCHITECTURE_SUB_STEPS:
        raise ValueError(f"unknown architecture section {sub_step!r}")
    schema = _architecture_section_schema(sub_step)
    apply_collection_bounds(schema, STAGE_COLLECTION_BOUNDS.get("architecture", ()))
    if sub_step == "parts":
        functional_spec = prompt_state.get("functional_spec")
        if isinstance(functional_spec, dict):
            spec = models.FunctionalSpec.model_validate(functional_spec)
            names = [block.name for block in spec.blocks]
            if names:
                requirement = (schema.get("$defs") or {}).get("IntentRequirement") or {}
                ownership = (requirement.get("properties") or {}).get("functional_blocks")
                if isinstance(ownership, dict) and isinstance(ownership.get("items"), dict):
                    ownership["items"]["enum"] = names
    return StageResponseContract(
        stage="architecture",
        schema=schema,
        contract_name=f"kicraft_architecture_{sub_step}_response_v1",
        allow_questions=allow_questions,
    )


def architecture_draft(raw: str) -> dict:
    """The fields a refused architecture answer managed to state ('' -> {})."""
    try:
    ...
    if not isinstance(parsed, dict):
        return {}
    return {key: value for key, value in parsed.items() if key != "questions"}


def architecture_section_fields(answer: dict) -> dict:
    """The architecture intent fields one section answer carries."""
    return {
        key: value
        for key, value in answer.items()
        if key in _ARCHITECTURE_INTENT_FIELDS
    }


def _unowned_source_obligations(assembled: dict, prompt_state: dict) -> bool:
    """Whether the committed obligations have no requirement owner in this draft."""
    try:
    ...
    owned = {
        (row["kind"], row["original_obligation_id"])
        for requirement in assembled.get("requirements") or []
        if isinstance(requirement, dict)
        for row in _canonical_obligations(requirement.get("obligations"))
    }
    return any(
        key not in owned and key[0] not in models.OWNERSHIP_EXEMPT_OBLIGATION_KINDS
        for key in ((row["kind"], row["original_obligation_id"]) for row in rows)
    )


def architecture_pending_sections(
    assembled: dict,
    prompt_state: dict,
    diagnostic: dict | None = None,
) -> tuple[str, ...]:
    """The sections this architecture answer still owes, in dependency order.

    A section is pending when the draft states nothing for it (an unparseable answer states nothing),
    and any section the refusal's own codes name follows in the same order, so the section that owns
    the defect is asked again with only that section's schema in front of it.
    """
    pending: set[str] = set()
    if not assembled.get("sheets"):
        pending.add("sheets")
    if not assembled.get("requirements"):
        pending.add("parts")
    if not assembled.get("signals"):
        pending.add("signals")
    if _unowned_source_obligations(assembled, prompt_state):
        pending.add("obligations")
    for code in architecture_rejection_codes(diagnostic):
        section = _ARCHITECTURE_CODE_SECTIONS.get(code)
        if section:
            pending.add(section)
    return tuple(section for section in ARCHITECTURE_SUB_STEPS if section in pending)


def apply_architecture_obligation_owners(
    assembled: dict,
    owners: object,
    prompt_state: dict,
) -> None:
    """Write each committed obligation row onto the requirements that implement it.

    The section answers ownership (`<original_obligation_id> -> <requirement_id>`); the row itself is
    the committed intent/functional_spec row, copied verbatim by the compiler. A malformed answer
    changes nothing, so the section is re-asked with its own refusal.
    """
    try:
        requested = ArchitectureObligationsResponse.model_validate({"owners": owners}).owners
        rows = {
            (row["kind"], row["original_obligation_id"]): row
            for row in _canonical_obligations(
                source_obligation_rows(prompt_state, slots=("intent", "functional_spec"))
            )
        }
    ...
    for owner in requested:
        row = rows.get((owner.kind, owner.original_obligation_id))
        if row is None:
            continue
        for requirement_id in owner.requirement_ids:
            requirement = requirements.get(requirement_id)
            if requirement is None:
                continue
            existing = {
                (item["kind"], item["original_obligation_id"])
                for item in _canonical_obligations(requirement.get("obligations"))
            }
            if (owner.kind, owner.original_obligation_id) in existing:
                continue
            requirement["obligations"] = [
                *(requirement.get("obligations") or []),
                row,
            ]
...
def assemble_architecture_sections(
    ...
    """Fold one section answer into the draft, ownership written by the compiler."""
    if sub_step == "obligations" and "owners" in answer:
        apply_architecture_obligation_owners(assembled, answer.get("owners"), prompt_state)
    for key, value in architecture_section_fields(answer).items():
        assembled[key] = value
    return assembled


def architecture_section_context(
    sub_step: str,
    assembled: dict,
    prompt_state: dict,
) -> dict:
    """The already-fixed facts one section call must see, and no more.

    Later sections reference earlier ids, so the section call is handed exactly the sections it
    depends on: the parts need the sheets and rails, the signals need the parts and their published
    ports, and ownership needs the committed obligation rows and the requirement ids.
    """
    if sub_step == "parts":
        return {
            key: assembled[key]
            for key in ("topologies", "comms_protocols", "mcu_present", "power", "sheets")
            if assembled.get(key)
        }
    if sub_step == "signals":
        return {
            "sheets": [
                sheet.get("name")
                for sheet in assembled.get("sheets") or []
                if isinstance(sheet, dict)
            ],
            "requirements": [
                {
                    key: requirement.get(key)
                    for key in ("id", "sheet", "family", "exact_part", "parameters", "declared_ports")
