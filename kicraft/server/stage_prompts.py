...
def _stage_extra(stage: str) -> str:
    ...
    if stage == "architecture":
        return (
            ...
            "current, frequency, pitch, and component values remain on the "
    ...
    if stage == "bom":
        return (
            ...
            "'Connector_PinHeader_2.54mm:PinHeader_2x08_P2.54mm_Vertical'); when results "
            "include a curated bundle, the bundle WINS over a stock footprint. For an ordinary "
            "1xNN pin header, use the verified symbol pattern Connector_Generic:Conn_01xNN "
            "with the matching Connector_PinHeader_2.54mm:"
            "PinHeader_1xNN_P2.54mm_Vertical footprint (NN is zero-padded, such as 02 or 09). "
            "For a plain test point use Connector:TestPoint; these exact stock families need "
            "no lookup. Connectors with a mating or orientation constraint (RF/coax: BNC, SMA, "
            "U.FL; USB; card sockets) are NOT generic: use a curated bundle, or resolve the real "
            "part with "
            ...
            "If a genuinely missing part remains, you have ONE optional tool-call round; batch "
            "every independent lookup together in that turn. The next response is final JSON, "
            "so never spend the lookup round verifying a result already in the prompt.\n"
...
_WORKED_EXAMPLES = {
    ...
    "bom": (
        ...
        '"value": "L78L33_TO92", "symbol": "Regulator_Linear:L78L33_TO92", '
        '"footprint": "Package_TO_SOT_THT:TO-92_Inline", "sheet": "POWER", '
        '"mpn": "L78L33ACZ"}, '
