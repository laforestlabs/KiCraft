...
def load_rows(db_path, since=None) -> list[dict]:
    ...
    try:
        q = ("SELECT id, ts, model, input_tokens, output_tokens, cost_usd, meta "
             "FROM spend")
        params: tuple = ()
        if since:
            q += " WHERE ts >= ?"
        ...
        for rid, ts, model, intok, outtok, cost, meta in cur.fetchall():
            m = _parse_meta(meta)
            rows.append({
                "id": rid, "ts": ts, "model": model,
                "input_tokens": int(intok or 0), "output_tokens": int(outtok or 0),
                "cost_usd": float(cost or 0.0), "meta": m,
...
def _bucket():
    return {"calls": 0, "input": 0, "output": 0, "cached": 0, "cost": 0.0,
            "spikes": 0, "truncations": 0}
...
def _add(b, r, spike_threshold):
    ...
    b["cost"] += r["cost_usd"]
    if r["input_tokens"] and (r["cost_usd"] / r["input_tokens"] * 1e6) > spike_threshold \
...
def format_report(summary, by="run") -> str:
    ...
    out = []
    out.append("=" * 72)
    out.append("  KiCraft web spend  ({} calls, {} runs)".format(
        t["calls"], len(summary["runs"])))
    out.append("=" * 72)
    out.append("  Total cost     ${:.4f}".format(t["cost"]))
    out.append("  Input tokens   {:>14,}   (cache hit-rate {:.1f}%)".format(
        t["input"], _hit_rate(t)))
    out.append("  Output tokens  {:>14,}".format(t["output"]))
    out.append("  Price spikes   {:>14}   (calls > ${:.2f}/Mtok input)".format(
        t["spikes"], summary["spike_threshold"]))
    out.append("  Truncations    {:>14}   (output hit the token cap)".format(
        t["truncations"]))
    ...
    if by in ("provider", "all"):
        ...
        for prov, b in sorted(summary["providers"].items(), key=lambda kv: -kv[1]["cost"]):
            out.append("    {:<18} ${:>8.4f}  {:>4} calls  hit {:>5.1f}%  spikes {}".format(
    ...
    if by in ("run", "stage", "all"):
        ...
        for run, b in sorted(summary["runs"].items(), key=lambda kv: -kv[1]["cost"]):
            out.append("    {:<26} ${:>8.4f}  {:>4} calls  hit {:>5.1f}%  spk {}  trunc {}".format(
                str(run)[:26], b["cost"], b["calls"], _hit_rate(b), b["spikes"], b["truncations"]))
            if by in ("stage", "all"):
                for stage, sb in sorted(summary["run_stage"][run].items(),
                                        key=lambda kv: -kv[1]["cost"]):
                    out.append("        {:<22} ${:>8.4f}  {:>3} calls  in {:>8,}  hit {:>5.1f}%".format(
...
def main(argv=None) -> int:
    ...
    ap.add_argument("ledger", nargs="?", default=str(DEFAULT_LEDGER),
                    help=f"path to spend_ledger.db (default: {DEFAULT_LEDGER})")
    ap.add_argument("--by", choices=["run", "stage", "provider", "all"], default="run",
    ...
    if not rows:
        print("no spend rows found" + (f" since {args.since}" if args.since else ""),
              file=sys.stderr)
        return 0
