...
Reminder: every *physical* block from the Functional Spec gets a Sheet — don't drop any (a `count`-N block expands to N grouped sheets). Functional-spec blocks whose `category` is `power` (`GND`, `+12V`, `VBUS`, a rail output, …) are power/ground NETS, not sheets: declare them in `power_nets` and never emit a Sheet for them. Every functional-spec `connection` either appears in `inter_sheet_nets` or is entirely local to one sheet if you reorganized.
