...
pytest.importorskip("pydantic")  # stage_driver imports the pydantic-backed models
from kicraft.server import stage_driver  # noqa: E402
...
def test_bom_executor_only_dispatches_whitelisted_tools(tmp_path):
    ...
    execute = stage_driver._bom_executor(tmp_path)
...
def test_executor_never_uses_shell_or_interpolates_args():
    ...
    src = inspect.getsource(stage_driver._bom_executor)
    assert "shell=True" not in src
    assert "_run(KICRAFT" in src  # all tool calls go through the argv-list runner
    run_src = inspect.getsource(stage_driver._run)
...
def test_tool_loop_is_bounded_against_runaway_calls():
    ...
    assert stage_driver._BOM_MAX_ROUNDS <= 8
