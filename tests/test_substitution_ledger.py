...
def _bom(parts, substitutions=(), assumptions=()):
    return BOM(parts=parts, substitutions=list(substitutions), assumptions=list(assumptions))
...
_RUN20_GOAL = (
    "A front-panel board: a through-hole rotary encoder with push "
...
def test_mount_type_contradiction_ledgered_passes():
    bom = _bom(
        ...
        substitutions=[
            Substitution(
                wanted="SMT I2C OLED",
                got="through-hole OLED HS96L03W2C03",
...
def test_mount_type_matching_footprint_passes():
    bom = _bom(
        [_part("U2", "SSD1306 OLED", symbol="Display:OLED", footprint="Display:OLED_SSD1306_SMD")]
    )
    assert check_mount_type_consistency(_run20_intent(), bom).ok
...
def test_mount_type_unclassifiable_footprint_skipped():
    ...
    bom = _bom([_part("BZ1", "Buzzer", symbol="Device:Buzzer", footprint="Buzzer:Buzzer_Generic")])
    assert check_mount_type_consistency(intent, bom).ok
...
def test_facts_report_access_parts_and_verdict():
    bom = _bom(
        [
            _part("U1", "RP2040", symbol="MCU:RP2040", footprint="Package_DFN_QFN:QFN-56"),
            _part(
                "J1",
                "USB-C receptacle",
