...
def test_delete_project_reaps_build_jobs_and_refuses_while_running(store, tmp_path):
    """delete_project closes the build_jobs row leak (rows were never deleted)
    and reaps terminal jobs' workspaces -- but REFUSES while a build is running:
    under build-in-place the project tree is the running build's cwd, so
    deleting would rmtree under a live worker subprocess."""
    ...
    assert claimed is not None and claimed.id == jid_done
    ...
    with pytest.raises(ValueError):
        store.delete_project(pid)                      # live build -> refused
    assert store.get_project(pid) is not None          # nothing was deleted

    store.finish_build(jid_running, rc=0, status="done")
    ...
    assert not ws_running.exists()                     # now terminal -> reaped too
