...
def _no_local_jlc_catalog(tmp_path_factory, monkeypatch):
    ...
    monkeypatch.setenv(
        "KICRAFT_JLCPARTS_DB", str(tmp_path_factory.mktemp("jlc") / "absent.sqlite3")
    )
...
def test_block_keeps_lcsc_less_series_rows_when_filtering(tmp_path, monkeypatch):
    ...
    block = _format_core_defaults_block([_row("good-block", "GOOD-IC", "C100"), row])
...
def test_drive_stage_injects_db_rows_overriding_nothing_else(tmp_path):
    edited = [
        {
            "function_key": "usb-uart-bridge",
            "display_name": "USB-UART bridge",
            "qualifier": "USB 2.0 FS",
            "default_mpn": "FT232RL-EDITED",
            "default_lcsc": "C52717",
            "package": "SSOP-28",
            "enabled": True,
        }
...
def test_bundle_row_dropped_when_cached_retail_dry(monkeypatch):
    ...
    rows = [
        {
            "function_key": "dc-motor-driver",
            "default_mpn": "DRV8833PWPR",
            "bundle": "drv8833",
            "enabled": True,
        },
        {
