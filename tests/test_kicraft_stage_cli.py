...
def test_stage_commit_architecture_rejects_empty_implementation_plan(tmp_path, capsys):
    state_path = tmp_path / "state.json"
    ...
    slot = _write_slot(tmp_path, "arch", architecture)
    rc, payload = _run(
        capsys,
        "stage-commit",
        "architecture",
    ...
    assert rc == 3
    assert payload["ok"] is False
    assert "no implementation requirements" in payload["errors"][0]


def test_stage_commit_architecture_requires_implementation_for_every_sheet(tmp_path, capsys):
    state_path = tmp_path / "state.json"
    state_path.write_text(
        json.dumps(
            {
    ...
    architecture = _valid_architecture()
    architecture["requirements"] = architecture["requirements"][:1]
    slot = _write_slot(tmp_path, "arch", architecture)
    ...
    assert rc == 3
    assert payload["ok"] is False
    assert "sheets with no implementation" in payload["errors"][0]
    assert payload["offenders"] == ["LDO"]


def test_stage_commit_architecture_rejects_incomplete_lowerer_contract(tmp_path, capsys):
    state_path = tmp_path / "state.json"
    state_path.write_text(
        json.dumps(
            {
    ...
    assert rc == 3
    assert payload["ok"] is False
    assert "complete lowerer contract" in payload["errors"][0]
    assert payload["offenders"][0].startswith("ladder (r2r_ladder)")


def test_stage_commit_architecture_binds_intent_named_parts(tmp_path, capsys):
    state_path = tmp_path / "state.json"
    intent = _valid_intent()
    intent["named_parts"] = ["MAX31855"]
    state_path.write_text(
        json.dumps(
            {
                "project_stem": "TEST",
    ...
    architecture["requirements"] = [
        {
            "id": "thermocouple_amplifier",
            "sheet": "MCU",
            "role": "sensor",
            "family": "thermocouple_amplifier",
        },
        {
            "id": "ldo_support",
            "sheet": "LDO",
    ...
    assert rc == 3
    assert payload["ok"] is False
    assert "does not bind every intent-named part" in payload["errors"][0]
    assert payload["offenders"] == ["MAX31855"]
