...
def test_emits_four_schema_valid_bom_parts():
    parts, _ = standard_form_factor_bom_delta(_t(), existing_refs=set())
    assert len(parts) == 4
...
def test_refs_do_not_collide_with_existing():
    parts, _ = standard_form_factor_bom_delta(
...
def test_refs_start_at_j1_when_no_connectors():
    parts, _ = standard_form_factor_bom_delta(_t(), existing_refs={"U1", "C1"})
    assert [p.ref for p in parts] == ["J1", "J2", "J3", "J4"]


def test_power_nets_bound_signal_nets_not():
    _, conns = standard_form_factor_bom_delta(_t(), existing_refs=set())
...
def test_connections_are_schema_valid_and_nc_excluded():
    parts, conns = standard_form_factor_bom_delta(_t(), existing_refs=set())
